...
from losses.face_losses import arcface_loss, softmax_loss
...
def get_parser():
	...
	parser.add_argument('--centre_weight', default=0.3, help='centre loss weight')
...
if __name__ == '__main__':
    ...
    logit, inner_sim = arcface_loss(embedding=net.outputs, labels=labels, w_init=w_init_method, out_num=args.num_output)
    ...
    train_acc = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(pred, axis=1), labels), dtype=tf.float32))
    # 3.10 define sess
    #sess = tf.Session()
    sess_config = tf.ConfigProto(allow_soft_placement=True, log_device_placement=args.log_device_mapping)
        ...
        for batch in range(batch_per_epoch):
            try:
                ...
                if count > 0 and count % args.show_info_interval == 0:
                    print('%2d, %d/%d, loss:%.2f , iloss:%.2f, wloss:%.2f, inner:%.2f, acc:%.4f, speed:%.1f' %
                          (i, batch, batch_per_epoch, total_loss_val, inference_loss_val, wd_loss_val,
                           inner_loss_val, acc_val, pre_sec))
                ...
                model_lfw = 0
                model_ytf = 0
                if config.get('lfw').enable and count > 0 and count % args.validate_interval == 0:
                ...
                # ytf validate
                if config.get('ytf').enable and count > 0 and count % args.validate_interval == 0:
                    feed_dict_test ={dropout_rate: 1.0}
                    feed_dict_test.update(tl.utils.dict_to_one(net.all_drop))
                    extractor = TensorflowExtractor(sess, embedding_tensor, args.batch_size, feed_dict, images)
